...
public class GroupMembershipDao {
    ...
    // DelegateDao called from the cache manager when there is no hit in the cache for the key
    // needs to receive a "cache-key-object", extract values from it and call the original method
    //       to access the data base
    // then call the original DAO method using the extracted values
    public boolean isUserInGroup(final GroupMembershipCacheKey theCacheKey) {
        return isUserInGroup(theCacheKey.getUserId(), theCacheKey.getGroupId()); // call original DAO method
    }
    /**
